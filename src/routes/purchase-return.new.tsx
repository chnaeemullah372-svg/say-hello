...
export const Route = createFileRoute("/purchase-return/new")({
  head: () => ({ meta: [
    { title: "Create Purchase Return — CN Invoice" },
    { name: "description", content: "Record goods returned to a supplier." },
  ]}),
...
function CreatePurchaseReturn() {
  ...
  const { customers, products, addCustomer, updateCustomer, addProduct, updateProduct, addPurchaseReturn, updatePurchaseReturn, purchaseReturns } = useStore();
  ...
  const editingReturn = useMemo(() => (editId ? purchaseReturns.find((p) => p.id === editId) : undefined), [editId, purchaseReturns]);
  ...
  useEffect(() => {
    if (editingReturn && loadedEditId !== editingReturn.id) {
      setSupplierId(editingReturn.supplierId);
      setItems(editingReturn.items.map((it) => ({ ...it })));
      setDiscountMode(editingReturn.discountMode ?? "rate");
      setDiscountValue(editingReturn.discountValue ?? 0);
      setTaxPct(editingReturn.taxRate);
      setShippingAmount(editingReturn.shippingAmount ?? 0);
      setStatus(editingReturn.status);
      setNotes(editingReturn.notes || "");
      setLoadedEditId(editingReturn.id);
    }
  }, [editingReturn, loadedEditId]);
  ...
  const buildDraftPdfDoc = async () => {
    ...
    doc.setFontSize(10);
    doc.text(editingReturn ? editingReturn.number : "Draft Purchase Return", 14, 23);
    doc.text(`Date: ${returnDate.toISOString().slice(0, 10)}`, 140, 16);
    doc.text(`Bill From: ${supplier?.name || ""}`, 14, 30);
    autoTable(doc, {
      startY: 36,
  ...
  const save = async (opts: { print?: boolean; send?: boolean } = {}) => {
    ...
    try {
      ...
      if (editingReturn) {
        const oldTotal = editingReturn.total;
        if (supplier) await updateCustomer(supplier.id, { payableBalance: (supplier.payableBalance ?? 0) - (total - oldTotal) });
        await applyStock(editingReturn.items, plainItems);
        await updatePurchaseReturn(editingReturn.id, {
          supplierId, items: plainItems, taxRate: effectiveTaxRate, discountMode, discountValue, shippingAmount, notes,
          total, status: status as PurchaseReturn["status"],
        });
        returnNumber = editingReturn.number;
        toast.success(`Purchase return ${editingReturn.number} updated`);
      } else {
        const ret = await addPurchaseReturn({
          supplierId, date: returnDate.toISOString().slice(0, 10), items: plainItems,
        ...
        toast.success(`Purchase return ${ret.number} saved`);
      }
      ...
      if (opts.send && supplier) {
        ...
        if (to.length) {
          sendAndLogWhatsApp({
            ...
            toNumbers: to,
            message: `Hello ${supplier.name}, purchase return ${returnNumber} of ${fmt(total)} has been recorded.`,
            messageType: "other",
            referenceId: editingReturn?.id ?? returnNumber,
      ...
      setTimeout(() => nav({ to: "/purchase-return" }), 150);
    } catch (err) {
      toast.error(err instanceof Error ? err.message : "Could not save purchase return");
      setSaving(false);
    }
  };
  ...
  return (
    <div className="-m-4 sm:-m-6 lg:-m-8 pb-28">
      <div className="sticky top-14 z-20 flex items-center gap-2 bg-primary px-3 py-2.5 text-primary-foreground shadow-sm">
        <button onClick={() => nav({ to: "/purchase-return" })} className="grid h-9 w-9 place-items-center rounded-full hover:bg-white/10" aria-label="Back">
          <ArrowLeft className="h-5 w-5" />
        </button>
        <div className="flex-1 text-center font-display text-base font-bold tracking-widest">
          {editingReturn ? "EDIT PURCHASE RETURN" : "PURCHASE RETURN"}
        </div>
        <button
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            <DropdownMenuItem onClick={async () => { if (!items.length) return toast.error("Add at least one item first"); const doc = await buildDraftPdfDoc(); doc.save(`${editingReturn ? editingReturn.number : "purchase-return-draft"}.pdf`); toast.success("PDF downloaded"); }}>
      ...
      <div className="mx-auto max-w-2xl bg-background">
        ...
        <div className="flex items-start justify-between gap-3 border-b bg-card px-4 py-4">
          <div className="font-display text-2xl font-black tracking-tight">
            {editingReturn ? editingReturn.number : "Auto-generated"}
          </div>
          <div className="text-right">
            <div className="text-[11px] uppercase tracking-wider text-muted-foreground">Return Date</div>
        ...
        <div className="border-b bg-card px-4 py-3">
          <Label className="text-[10px] uppercase tracking-wider text-muted-foreground">Return Status</Label>
        ...
        <div className="border-b bg-card">
          ...
          {termsOpen && (
            <div className="border-t px-4 py-3">
              <Textarea rows={3} value={terms} onChange={(e) => setTerms(e.target.value)} placeholder="Terms for this return…" autoFocus />
            </div>
          )}
        </div>
        ...
        <DialogContent className="max-w-md p-0">
          ...
          <Command>
            ...
            <CommandList className="max-h-[50vh]">
              ...
              <CommandGroup>
                {suppliers.map((c) => (
                  <CommandItem key={c.id} value={c.name} onSelect={() => { setSupplierId(c.id); setCustOpen(false); }}>
