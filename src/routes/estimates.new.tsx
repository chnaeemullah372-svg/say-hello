  ...
  const save = async (opts: { print?: boolean; send?: boolean } = {}) => {
    ...
    try {
      ...
      if (editingEstimate) {
        ...
        if (user?.tenantId) {
          try {
            const { data } = await supabase.rpc("next_document_number", { p_doc_type: "estimate", p_tenant_id: user.tenantId });
            if (data) explicitNumber = data as string;
          } catch { /* fall back to the DB's own numbering if the RPC isn't available */ }
        }
        const est = await addEstimate({ ...payload, number: explicitNumber } as any);
