...
function CreateEstimate() {
  ...
  // `editingEstimate` (loads asynchronously from `estimates`, so on an EDIT
  // page's first render it's still undefined — guarding on it would let
  // this effect's `.then()` land after the real invoice loads and stomp its
  // actual saved terms with the tenant's default).
  useEffect(() => {
    if (editId) return;
  ...
  const buildDraftPdfDoc = async () => {
    ...
    const docData = {
      ...
      dateValue: estimateDate.toISOString().slice(0, 10),
      secondDateLabel: "Valid Until",
      secondDateValue: validUntil || undefined,
      businessName,
