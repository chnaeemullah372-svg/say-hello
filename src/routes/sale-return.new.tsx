...
export const Route = createFileRoute("/sale-return/new")({
  head: () => ({ meta: [
    { title: "Create Sale Return — CN Invoice" },
    { name: "description", content: "Record goods returned by a customer." },
  ]}),
  component: CreateSaleReturn,
});
...
function CreateSaleReturn() {
  ...
  const { customers, products, addCustomer, updateCustomer, addProduct, updateProduct, addSaleReturn, updateSaleReturn, saleReturns } = useStore();

  const editId = useMemo(() => {
    if (typeof window === "undefined") return null;
    return new URLSearchParams(window.location.search).get("edit");
  }, []);
  const editingReturn = useMemo(() => (editId ? saleReturns.find((s) => s.id === editId) : undefined), [editId, saleReturns]);
  ...
  useEffect(() => {
    if (editingReturn && loadedEditId !== editingReturn.id) {
      setCustomerId(editingReturn.customerId);
  ...
  const buildDraftPdfDoc = async () => {
    ...
    doc.setFontSize(10);
    doc.text(editingReturn ? editingReturn.number : "Draft Sale Return", 14, 23);
  ...
  // AND brings the goods back into stock.
  ...
  const save = async (opts: { print?: boolean; send?: boolean } = {}) => {
    ...
    try {
      ...
      if (editingReturn) {
        ...
        if (customer) await updateCustomer(customer.id, { balance: customer.balance - (total - oldTotal) });
        await applyStock(editingReturn.items, plainItems);
        await updateSaleReturn(editingReturn.id, {
          customerId, items: plainItems, taxRate: effectiveTaxRate, discountMode, discountValue, shippingAmount, notes,
          total, status: status as SaleReturn["status"],
        });
        returnNumber = editingReturn.number;
        toast.success(`Sale return ${editingReturn.number} updated`);
      } else {
        const ret = await addSaleReturn({
          customerId, date: returnDate.toISOString().slice(0, 10), items: plainItems,
        ...
        toast.success(`Sale return ${ret.number} saved`);
      }
      ...
      if (opts.send && customer) {
        ...
        if (to.length) {
          sendAndLogWhatsApp({
            ...
            toNumbers: to,
            message: `Hello ${customer.name}, sale return ${returnNumber} of ${fmt(total)} has been recorded.`,
            messageType: "other",
      ...
      setTimeout(() => nav({ to: "/sale-return" }), 150);
    } catch (err) {
      toast.error(err instanceof Error ? err.message : "Could not save sale return");
      setSaving(false);
    }
  };
  ...
  return (
    <div className="-m-4 sm:-m-6 lg:-m-8 pb-28">
      <div className="sticky top-14 z-20 flex items-center gap-2 bg-primary px-3 py-2.5 text-primary-foreground shadow-sm">
        <button onClick={() => nav({ to: "/sale-return" })} className="grid h-9 w-9 place-items-center rounded-full hover:bg-white/10" aria-label="Back">
          <ArrowLeft className="h-5 w-5" />
        </button>
        <div className="flex-1 text-center font-display text-base font-bold tracking-widest">
          {editingReturn ? "EDIT SALE RETURN" : "SALE RETURN"}
        </div>
        <button
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            <DropdownMenuItem onClick={async () => { if (!items.length) return toast.error("Add at least one item first"); const doc = await buildDraftPdfDoc(); doc.save(`${editingReturn ? editingReturn.number : "sale-return-draft"}.pdf`); toast.success("PDF downloaded"); }}>
